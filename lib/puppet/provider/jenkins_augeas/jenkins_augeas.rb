...
#
#
# This is a fork of the augeas type of puppet 3.4
# I added some options etc. I replace this code later with a rewrite,
# but for now it is goot enough.
#
#
#
#
...
# required libs for encryption and decryption
require 'openssl'
require 'digest/sha2'
require "base64"
require 'optparse'
...
Puppet::Type.type(:jenkins_augeas).provide(:jenkins_augeas) do
  include Puppet::Util
  ...
  JENKINS_COMMANDS = {
    "set" => [ :path, :string ],
    ########################################
    # eset for encrypted strings
    "eset" => [ :path, :crypted_string ],
    # cset set an empty node
    "cset" => [ :path ],
    "touch" => [ :path ],
    ########################################
  ...
  JENKINS_COMMANDS["ins"] = JENKINS_COMMANDS["insert"]
  JENKINS_COMMANDS["remove"] = JENKINS_COMMANDS["rm"]
  JENKINS_COMMANDS["move"] = JENKINS_COMMANDS["mv"]

  attr_accessor :aug

  def self.prefetch(resources)
    master_key_file_path = resources.first[1][:master_key_file_path]
    hudson_util_secret_file_path = resources.first[1][:hudson_util_secret_file_path]
    generate_secret(master_key_file_path, hudson_util_secret_file_path)
  end
  ...
  def parse_commands(data)
    ...
    master_key_file_path = resource[:master_key_file_path]
    hudson_util_secret_file_path = resource[:hudson_util_secret_file_path]
    ...
    data.each do |line|
      ...
      cmd = sc.scan(/\w+|==|!=/)
      formals = JENKINS_COMMANDS[cmd]
      fail("Unknown command #{cmd}") unless formals
      ########################################
      # This is the change for jenkins, so I can set the encrypted values
      if (cmd == "eset" or cmd == "cset")
        argline << "set"
      else
        argline << cmd
      end
      ...
      formals.each do |f|
        ...
        ########################################
        # Modification for jenkins so I can set encrypted values
        elsif f == :crypted_string
          delim = sc.peek(1)
          if delim == "'" || delim == "\""
            sc.getch
            argline << encrypt_base64(master_key_file_path, hudson_util_secret_file_path, sc.scan(/([^\\#{delim}]|(\\.))*/))
            sc.getch
          else
            argline << encrypt_base64(master_key_file_path, hudson_util_secret_file_path, sc.scan(/[^\s]+/))
          end
          fail("missing string argument #{narg} for #{cmd}") unless argline[-1]
        ########################################
        elsif f == :comparator
  ...
  # Generates hudson.util.Secret if it is necessary
  def self.generate_secret(master_key_file_path, hudson_util_secret_file_path)
    if !File.exists?(hudson_util_secret_file_path)
      master_key = open(master_key_file_path).readlines[0]
      hudson_util_secret = Random.new.bytes(256)+MAGIC
      hashed_master_key = (Digest::SHA2.new << master_key).digest[0..15]
      aes = OpenSSL::Cipher::Cipher.new("AES-128-ECB")
      aes.encrypt
      aes.key = hashed_master_key
      jenkins_encrypted_key = aes.update(hudson_util_secret) + aes.final
      File.open(hudson_util_secret_file_path, mode='w') do |file|
        file.write(jenkins_encrypted_key)
        file.flush
      end
      jenkins_encrypted_key
    end
  end
